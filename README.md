...
→ `CRUD`의 Create(생성), Read(읽기), Update(갱신), Delete(삭제)를 가능하게 하는 기능 모두 구현.

→ `Codable`을 이용하여 Json 파일로 저장하여 연락처가 실시간으로 저장되게 하였다.
...
`SpriteKit`와 `DispatchQueue`를 같이 사용하는 것은 서버에서 데이터를 불러오는 일이 아닌 이상 최대한 피하라고 애플 개발자 포럼 홈페이지에 많은 이들이 답변 하였다.

애플은 대부분의 모션 동작이 있는 게임을 `SpriteKit`를 활용하여 구현한다. 엄청난 노가다 검색 끝에 알게된 사실이 있다. 
...
충돌을 감지하는 것을 구현하는것은 엄청난 노가다 일 줄 알았는데 `intersects`라는 아주~ 편리한 함수가 있어서 비교적 쉽게 구현하였다 ㅎㅅㅎ
