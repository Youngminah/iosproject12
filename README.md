...
→ CRUD의 Create(생성), Read(읽기), Update(갱신), Delete(삭제)에 해당하는 내용을 모두 구현.

→ Codable을 이용하여 Json 파일로 저장하여 연락처가 저장되게 하였다.
...
→ SprintKit을 사용하지 않고, 애니메이션과 Multithread GCD를 이용하여 구현한 게임 앱.
